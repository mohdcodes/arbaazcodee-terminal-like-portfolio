...
import { X, Send, Loader2, Bot, Mic, Sun, Moon, Sparkles, Settings, MessageSquare, Zap } from 'lucide-react';
...
export const AiChatbot: React.FC<AiChatbotProps> = ({ onClose }) => {
  const [messages, setMessages] = useState<Message[]>([
    { 
      id: '1',
      role: 'assistant', 
      content: "Hi there! I'm Arbaaz's AI assistant. Ask me anything about Arbaaz, his projects, skills, or experience, or switch to general mode for broader questions!", 
      timestamp: new Date(),
    }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: "I apologize, but I'm having trouble processing your request. Please try again or switch chat modes.",
        timestamp: new Date(),
        parentId: messageId
      };
  ...
  const toggleChatMode = () => {
    ...
    setMessages(prev => [
      ...prev,
      {
        ...
        role: 'assistant',
        content: `Switched to ${chatMode === 'personal' ? 'general' : 'personal'} mode. ${
          chatMode === 'personal' 
  ...
  const getThemeClasses = () => {
    ...
    const modeClasses = themeMode === 'dark' 
  ...
  return (
    <div className={`flex flex-col h-full relative ${getThemeClasses()}`} onClick={handleChatbotClick}>
      {/* Animated background */}
      ...
      {/* Header */}
      <div className={`terminal-header p-3 flex items-center justify-between z-10 ${
        themeMode === 'dark' ? 'bg-gray-800' : 'bg-white'
      }`}>
        <div className="flex items-center">
          <Bot size={18} className={`mr-2 ${themeColor === 'blue' ? 'text-blue-400' : themeColor === 'purple' ? 'text-purple-400' : themeColor === 'green' ? 'text-green-400' : themeColor === 'orange' ? 'text-orange-400' : 'text-pink-400'}`} />
          <span className={`text-sm ${themeMode === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
            AI Assistant ({chatMode === 'personal' ? 'Personal Mode' : 'General Mode'})
          </span>
        </div>
        <div className="flex items-center space-x-2">
          <button 
            ...
            title={`Switch to ${chatMode === 'personal' ? 'General' : 'Personal'} Mode`}
          >
            {chatMode === 'personal' ? (
          ...
          <button 
            ...
            title="Settings"
          >
            <Settings size={16} className={themeMode === 'dark' ? 'text-gray-400' : 'text-gray-600'} />
          </button>
          <button 
            onClick={onClose}
            className={`p-1.5 rounded-full transition-colors ${
              themeMode === 'dark' ? 'hover:bg-gray-700 text-gray-400 hover:text-white' : 'hover:bg-gray-200 text-gray-600 hover:text-gray-900'
            }`}
      ...
      <AnimatePresence>
        {showSettings && (
          <motion.div 
            ...
            className={`overflow-hidden ${themeMode === 'dark' ? 'bg-gray-800' : 'bg-gray-100'} border-b ${themeMode === 'dark' ? 'border-gray-700' : 'border-gray-300'}`}
          >
            <div className="p-4">
              <h3 className={`text-sm font-medium mb-2 ${themeMode === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Theme Color</h3>
      ...
      {/* Messages */}
      <div className={`flex-1 p-4 overflow-y-auto terminal-scrollbar terminal-content relative z-10 ${
            ...
            >
              <div className="flex flex-col">
                <div className="flex items-center mb-1">
                  <span className={`text-xs ${themeMode === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                    {message.role === 'assistant' ? 'AI Assistant' : 'You'}
                  </span>
                  <span className={`text-xs ml-2 ${themeMode === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>
                ...
                {message.reactions && message.reactions.length > 0 && (
                  <div className="flex mt-2 flex-wrap gap-1">
                    {message.reactions.map((emoji, index) => (
                      <span key={index} className="text-sm bg-gray-800/30 rounded-full px-1.5 py-0.5">
                        {emoji}
                      </span>
                    ))}
                  </div>
                ...
                <div className="absolute bottom-1 right-1 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    onClick={() => {
                      ...
                      setShowEmojiPicker(!showEmojiPicker && message.id === activeMessageId);
                    }}
                    className={`p-1 rounded-full ${
                      themeMode === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-200 hover:bg-gray-300'
                    } text-xs`}
                  >
                    😀
                  </button>
            ...
            <div className="flex items-center">
              <div className="flex space-x-1 mr-2">
                <motion.div
                  ...
                  className={`w-2 h-2 rounded-full ${themeColor === 'blue' ? 'bg-blue-400' : themeColor === 'purple' ? 'bg-purple-400' : themeColor === 'green' ? 'bg-green-400' : themeColor === 'orange' ? 'bg-orange-400' : 'bg-pink-400'}`}
                />
                <motion.div
                  animate={{ y: [0, -5, 0] }}
      ...
      {/* Input area */}
      <div className={`terminal-input-container p-3 relative z-10 ${
        themeMode === 'dark' ? 'bg-gray-800' : 'bg-white'
      }`}>
        <form onSubmit={handleSubmit} className="flex items-center">
          ...
          <button
            ...
            className={`p-3 ${
              listening 
                ...
                : `${themeColor === 'blue' ? 'bg-blue-500 hover:bg-blue-600' : 
                    themeColor === 'purple' ? 'bg-purple-500 hover:bg-purple-600' : 
                    themeColor === 'green' ? 'bg-green-500 hover:bg-green-600' : 
                    themeColor === 'orange' ? 'bg-orange-500 hover:bg-orange-600' : 
                    'bg-pink-500 hover:bg-pink-600'}`
            } text-white transition-colors`}
            disabled={isLoading || !browserSupportsSpeechRecognition}
            title={browserSupportsSpeechRecognition ? "Voice input" : "Browser doesn't support voice input"}
          >
          ...
          <button
            ...
            className={`p-3 rounded-r-md ${
              ...
              themeColor === 'purple' ? 'bg-purple-600 hover:bg-purple-700' : 
              themeColor === 'green' ? 'bg-green-600 hover:bg-green-700' : 
        ...
        {/* Mode indicator */}
        <div className={`absolute -top-6 left-1/2 transform -translate-x-1/2 px-3 py-1 rounded-t-lg text-xs font-medium ${
          chatMode === 'personal'
