...
export const Terminal: React.FC<TerminalProps> = ({ 
  ...
  const formatLine = (line: string) => {
      ...
      if (parts[0] === 'arbaazcode' && parts.length > 1) {
        return (
          <>
            ...
            <span> </span>
            <span className="command-subcommand">{parts.slice(1).join(' ')}</span>
          </>
        );
      }
    ...
    if (line.includes('Technical Skills:') || line.includes('Education:') || line.includes('Contact Information')) {
            ...
            >
              {history.map((line, index) => (
                <motion.div 
                  ...
                  transition={{ duration: 0.2, delay: 0.05 * Math.min(index, 5) }}
                >
                  {formatLine(line)}
                </motion.div>
              ))}
